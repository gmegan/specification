...
\ac{PE} number 0. \oldtext{\acp{PE} are always ordered by the existing global \ac{PE} number that
...
Teams are easily represented by a (start, stride, size) triplet; however, implementations
are not required to limit a team's representation to such a triplet.  In some instances,
it may be advantageous to explicitly store a table of PE identifiers that map the team
numbers to the world or parent numbers...}
